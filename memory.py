...
class SegmentTree():
  ...
  def save_memory(self, filename):
    mkdir("", filename)
    with open(filename + '/data.npy', 'wb') as f:
      np.save(f, self.data)
    with open(filename + '/index.txt', 'w') as f:
      f.write("{}".format(self.index))
    print("save buffer to {}-index{}".format(filename, self.index))
  
  def load_memory(self, filename):
    with open(filename + '/data.npy', 'rb') as f:
       self.data = np.load(f)
    with open(filename + '/index.txt', 'r') as f:
      self.index = int(f.read())
    print("loaded memory from {} ".format(filename))
...
class ReplayMemory():
  ...
  def __next__(self):
    ...
    return state
  def save_memory(self, filename):
      self.transitions.save_memory(filename)
  def load_memory(self, filename):
      self.transitions.load_memory(filename)
